RSpec.describe ServerlessHelpers do
  ...
  it 'allows event parser config via symbols' do
    described_class.config.event_parser = :yaml
    expect(described_class.config.event_parser).to be_kind_of ServerlessHelpers::Event::Parser::Yaml
  end

  it 'raises NameError for unfound parsers' do
    expect{described_class.config.event_parser = :does_not_exist}.to raise_error(NameError)
  end

  it 'raises ArgumentError for mismatch parsers' do
    expect{described_class.config.event_parser = String.new()}.to raise_error(ArgumentError)
  end
end
